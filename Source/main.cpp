...
BOOL CALLBACK StatisticsBox(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg){
		case WM_INITDIALOG:{
			...
			SetDlgItemText(hDlg, ID_NAME, g_cCastleGame.GetMapName());
			}return FALSE;
		case WM_COMMAND:
...
BOOL MainKeyboardProc(HWND hwnd, int nVirtKey, LPARAM lKeyData)
{
	...
	switch(nVirtKey){
		case '1': 
			if(g_cCastleGame.SendInput(1))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '2': 
			if(g_cCastleGame.SendInput(2))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '3': 
			if(g_cCastleGame.SendInput(3))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '4': 
			if(g_cCastleGame.SendInput(4))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '5': 
			if(g_cCastleGame.SendInput(5))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '6': 
			if(g_cCastleGame.SendInput(6))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '7': 
			if(g_cCastleGame.SendInput(7))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '8': 
			if(g_cCastleGame.SendInput(8))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		case '9': 
			if(g_cCastleGame.SendInput(9))
				RedrawWindow(hwnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
			break;
		default:return FALSE;
	}
...
LRESULT CALLBACK MainWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		case WM_PAINT:{
			...
			for(int i=0; i<g_cCastleGame.GetNumOutputLines(); i++)
			{
				char* szTemp = g_cCastleGame.GetOutputLine(i);
